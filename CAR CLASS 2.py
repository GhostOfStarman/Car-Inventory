...
class carInv:
    ...
    def __mul__(self, price):
        grandTot = (self._price * 1.10) + 600 #tax rate plus fees
        ...
        for row in outData:
            try:
                if noEnter in row[0]:
                    ...
                    print('Model, Make, Year, Vin, Condition, Color, Interior Color, Title Status,')
                    print('Transmission type, Engine type, Drive type, Mileage, Price,\n')

                    while True:
                        choice = input('Which detail would you like to know? (Type "Exit" to exit) ').lower()

                        if choice == 'model' or choice == 'make' or choice == 'year':
                            print(row[1])
                            continue
                        elif choice == 'vin':
                            print(row[2])
                        ...
                        if choice == 'color':
                            print(row[4])
                            continue
                        if choice == 'interior color':
                            print(row[5])
                            continue
                        if choice == 'title status':
                            print(row[6])
                            continue
                        if choice == 'transmission type':
                            print(row[7])
                            continue
                        if choice == 'engine type':
                            print(row[8])
                            continue
                        if choice == 'drive type':
                            print(row[9])
                            continue
                        if choice == 'mileage':
                            print(row[10])
                ...
                print('That Inventory Serial does not exist in this inventory')
                next_step = input('Continue? Yes/No? ').lower()
                if next_step == 'yes':
                    mainMenu()
                if next_step == 'no':
                    exit()
